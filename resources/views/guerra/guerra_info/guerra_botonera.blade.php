...
<form method="POST" action="{{ url("/guerra/{$guerra->id}/reiniciar") }}">
    @csrf
    <button type="submit" class="btn btn-outline-warning mb-3"><i class="bi bi-arrow-clockwise"></i> Reiniciar</button>
</form>
@endif
<div class="d-flex justify-content-center justify-content-md-end gap-2 mb-3">
    @if($guerra->estado == "Creado")
        ...
        <form method="POST" action="{{ url("/guerra/{$guerra->id}/estado") }}">
            @csrf
            <button type="submit" class="btn btn-outline-info"><i class="bi bi-play-fill"></i> Empezar</button>
        </form>
        @endif
    <a class="btn btn-outline-info" href="{{ url('/equipo/nuevo') }}"><i class="bi bi-plus-circle-fill"></i> Equipo</a>
    @else
